...
Important, we're working only on hoodie test and the rpc is https://rpc.hoodi.ethpandaops.io/ so use that for the entire process of the trap creation wherever needed. This means the ideas you will generate in phase 0 must be something that will exactly work on the hoodie testnet.
